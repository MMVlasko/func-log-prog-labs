...
rm_last_three_elems([H1 | T1], [H1 | T2]) :- 
    rm_last_three_elems(T1, T2).

% удаление последних трёх элементов с использованием стандартных предикатов
rm_last_three_elems_with_std_preds(X, []) :- 
    my_length(X, L), 
    L < 4.
rm_last_three_elems_with_std_preds([H1 | T1], [H1 | T2]) :- 
    rm_last_three_elems_with_std_preds(T1, T2).
